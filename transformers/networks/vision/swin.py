...
class Swin(nn.Module):
    def __init__(self, img_size: int, patch_size: int, num_classes: int, channels: int, num_layers: int, d_model: int,
        ...
        """
        Swin Transformer from "Swin Transformer: Hierarchical Vision Transformer using Shifted Windows" (https://arxiv.org/abs/2103.14030)
...
if __name__ == '__main__':
    ...
    # Model and optimizer
    model = Swin(img_size, patch_size, num_classes, channels, num_layers, dim_model, num_heads, dropout)
    ...
    history = model.fit_generator(train_loader, valid_loader, epochs=epoch, callbacks=[
        pt.ModelCheckpoint('logs/swin_best_epoch_{epoch}.ckpt', monitor='val_acc', mode='max', save_best_only=True,
